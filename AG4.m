...
for i=1:1000
        ...
        for k=1 : (np/4)
                ...
                n=randi([2,nbits3-1]); %Punto de cruce aleatorio
                hijobin3(2*k-1,:)= [padresbin3(2*k-1,1:n) padresbin3(2*k,n+1:nbits3)];
                hijobin3(2*k,:)= [padresbin3(2*k,1:n) padresbin3(2*k-1,n+1:nbits3)];
        end
        ...
        p=rand();
        if p>0.90
            nhijo3=randi(np/2); %hijo al azar
            bit3=randi(nbits3); %bit al azar
        ...
        hijoent3=bi2de(hijobin3);
        hijoreal3=(xmax3-xmin3)*hijoent3/(2^nbits3-1)+xmin3;
        ...
        x3= [cromosoma(np/2+1:np,6);hijoent3]; %Actualizacion de x1
        x3real= [cromosoma(np/2+1:np,7);hijoreal3];
end
plot(yprom);
y=-((x1real.^2 + x2real.^2 + x3real.^2));
cromosoma= [y x1 x1real x2 x2real x3 x3real];
[val,ind]=max(y)

disp(['Resultado: x1= ' num2str(cromosoma(ind,3)) ' x2= ' num2str(cromosoma(ind,5)) ' x3= ' num2str(cromosoma(ind,7)) ' y= ' num2str(val)])